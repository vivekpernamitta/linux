...
//#include <uapi/asm-generic/errn-base.h>
#include <uapi/asm-generic/errno.h>
// To include completion operation
...
struct completion completion_vdev;
...
{
	// completion
	wait_for_completion(&completion_vdev);
	if (inuse)
	{
...
static int char_dev_write(struct file *file, const char* buf, size_t lbuf, loff_t *ppos)
{
	...
	nbytes = lbuf - copy_from_user(char_device_buf, buf, lbuf);
	*ppos += nbytes;
//	msleep(2);
...
static int __init char_dev_init(void)
{
	...
	/* 
	completion best use for synchronous use 
	...
	*/
	init_completion( &completion_vdev );
	msleep(1000);
	/* 
 	 completion done i.e it will allow wait_for_completion to go further
	 if you have multiple functions use complete_all()
	*/
	complete(&completion_vdev);
	return 0;
