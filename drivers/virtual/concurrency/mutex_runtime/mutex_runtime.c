...
#define LINE_SAPERATION	for(i=0; i <15;i++) {printk("_______");}printk("\n");

static char  char_device_buf[MAX_LENGTH];
static struct cdev *vivek_cdev;
static dev_t mydev;
static int i;
...
static char * data_copy_to_dynamic_buffer(int nbytes){
	
	LINE_SAPERATION;		
	int ret=0;
	char  * dy_buffer = kmalloc(4004,GFP_KERNEL);
	ret = 	strncpy(dy_buffer, char_device_buf, nbytes);
	if (ret <= 0) 
		{ printk(KERN_ERR " Data not copied\n");}

	printk("[Function] = %s\n",  __func__);	
	printk( KERN_INFO "\t WRITING function, nbyes=%d, dy_buffer [address] = %p \n , DATA=%s\n", nbytes ,dy_buffer, char_device_buf);
...
//int nbytes = lbuf - copy_from_user(char_device_buf+ *ppos, buf, lbuf);
	...
	*ppos += nbytes;
	...
	data_copy_to_dynamic_buffer(nbytes);
