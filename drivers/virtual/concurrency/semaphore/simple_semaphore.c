...
// to include semaphore operation
...
struct semaphore semaphore_vdev;
...
{
	// semaphore lock , only one device can open at a time
	down(&semaphore_vdev);
	if (inuse)
	{
...
static int char_dev_release(struct inode *inode, struct file* file)
{
	...
	// semaphore release , now other process can access 
	up (&semaphore_vdev);
	return SUCCESS;
